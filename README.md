...
### 埋め込みモデル：multilingual-e5-small（本番想定: ruri-v3-310m）

当初 ruri-v3-310m（JMTEB最良クラス）を採用したが、CPU環境では226秒/バッチ（644チャンクで約5時間）となり現実的でなかった。
ポートフォリオ実行用途として `intfloat/multilingual-e5-small` にフォールバック。
**プレフィックスが異なる（文書側: `passage: ` / クエリ側: `query: `）**点が最大の既知の罠。
付与漏れをユニットテスト（`tests/test_prefix.py`）で検査する。

本番（GPU環境）では ruri-v3-310m + `文章: ` / `クエリ: ` に切り替えること。
...
| ruri-v3-310m（本番用） | CPU環境で約5時間/ingest（226秒/バッチ）。GPU必須。ポートフォリオ実行はmultilingual-e5-smallで代替 |
